...
import { initDatabase, getDatabase } from "./lib/ourin-database.js";
...
async function startConnection(options = {}) {
  try {
    ...
    colors.logger.warn("database", `inisialisasi database dilewati: ${error.message}`);
  }
  ...
  const sock = makeWASocket({
    version: version || [2, 3000, 1034074495],
    logger,
    printQRInTerminal:
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    try {
      ...
      colors.logger.warn("pairing", `gagal: ${error.message}`);
      colors.logger.info("pairing", "mencoba mode QR sebagai fallback");
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          ...
          const saluranName =
            config.saluran?.name || config.bot?.name || "anzzzdecoding";

          const welcomeText =
            `👋 *ʜᴀɪ, sᴀʟᴀᴍ ᴋᴇɴᴀʟ!*\n\n` +
            `Aku *${config.bot?.name || "anzzzdecoding"}* 🤖\n\n` +
            ...
            `╰┈┈⬡\n\n` +
            `> Gabung grup official: https://chat.whatsapp.com/GkwxFttTXFE9qjBjTwE7aj?s=cl&p=a&ilr=0&amv=3\n` +
